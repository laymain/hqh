...
func _setup_markers(markers: Array):
    for marker in markers:
        if marker.type == "START":
            ...
            add_child(player)
            player.set_spawn_position(_marker_to_world_position(marker))
        elif marker.type == "ENCOUNTER":
            var enemy : Node = enemy_prefab.instantiate()
            var enemy_character := enemy.find_child("Character") as Character
            add_child(enemy)
            enemy_character.global_position = _marker_to_world_position(marker) + Vector3(0, enemy_character.global_position.y, 0)
